...
pub trait Mirror1: Sized {
    ...
    fn as_member(self) -> <Self::Constructor as TypeConstructor1>::Of<Self::A> {
        fn witness<F: TypeConstructor1, A>(fa: <F as AppliedTypeConstructor1<A>>::Mirror) -> F::Of<A> {
            fa
        }
...
pub trait Mirror2: Sized {
    ...
    fn as_member(self) -> <Self::Constructor as TypeConstructor2>::Of<Self::A, Self::B> {
        fn witness<F: TypeConstructor2, A, B>(fa: <F as AppliedTypeConstructor2<A, B>>::Mirror) -> F::Of<A, B> {
            fa
        }
